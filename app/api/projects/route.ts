...
export async function POST(req: Request) {
  ...
  if (!idea || typeof idea !== "string" || idea.trim().length < 10) {
    return NextResponse.json(
      {
        error:
          "Опишите идею хотя бы одним предложением (от 10 символов)",
      },
      { status: 400 }
    );
  }
  ...
  if (idea.length > 2000) {
    return NextResponse.json(
      {
        error: "Слишком длинное описание (макс. 2000 символов)",
      },
      { status: 400 }
    );
  }
  ...
  try {
      ...
      return NextResponse.json(
        {
          error:
            parsed.reason ||
            "Эта идея не подходит для анализа. Попробуйте сформулировать легальную бизнес-идею.",
        },
        { status: 422 }
      );
    }
    ...
    return NextResponse.json(
      {
        error: "AI временно недоступен. Попробуйте ещё раз.",
      },
      { status: 502 }
    );
  }
